import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import ast
from pathlib import Path
from matplotlib.lines import Line2D

# --- Config: CSV files and which columns to plot ---
csv_files = {
    "Per Location": "plotting/csv_data/cummulative_errors_per_location.csv",
    # "Per Profile": "plotting/csv_data/cummulative_errors_per_profile.csv"
}

# Clear, academic labels
error_labels = {
    "errors": "Sum of Squared Errors (SSE)",
    "ldc_errors": "Load Duration Curve RMSE"
}

# --- Asset color mapping ---
asset_colors = {
    "Demand": "red",
    "Solar": "gold",
    "Offshore": "darkblue",
    "Onshore": "lightblue",
}

output_folder = Path("plots")
output_folder.mkdir(exist_ok=True)

def load_column_as_matrix(df, column_name):
    if column_name not in df.columns:
        raise ValueError(f"Column '{column_name}' not found in DataFrame.")
    return np.array(df[column_name].apply(ast.literal_eval).tolist())

# --- Plotting ---
for error_col in error_labels:
    plt.figure(figsize=(10, 6))

    asset_types_present = set()

    for label, csv_file in csv_files.items():
        df = pd.read_csv(csv_file)

        try:
            matrix = load_column_as_matrix(df, error_col)
        except ValueError:
            print(f"Skipping {label} ({error_col}) because column is missing.")
            continue

        mean_error = np.mean(matrix, axis=0)
        std_error = np.std(matrix, axis=0)
        x = np.arange(matrix.shape[1])

        # --- Plot individual lines colored by asset ---
        for i, row in enumerate(matrix):
            asset_type = df.loc[i, "asset"].split("_")[-1] if "asset" in df.columns else None
            color = asset_colors.get(asset_type, "gray")

            if asset_type:
                asset_types_present.add(asset_type)

            plt.plot(x, row, alpha=0.3, color=color)

        # --- Plot mean in black ---
        plt.plot(
            x,
            mean_error,
            color="black",
            linewidth=2.5,
            label=f"{label} Mean"
        )

    # --- Create custom legend entries for assets ---
    asset_legend_handles = [
        Line2D([0], [0], color=asset_colors[a], lw=2, label=a)
        for a in sorted(asset_types_present)
        if a in asset_colors
    ]

    mean_handle = Line2D([0], [0], color="black", lw=2.5, label="Mean")

    plt.xlabel("Number of HC Merges")
    plt.ylabel(error_labels.get(error_col, error_col))
    plt.title(f"{error_labels.get(error_col, error_col)} vs Number of Merges")
    plt.grid(True)

    # Combine legends cleanly
    plt.legend(handles=asset_legend_handles + [mean_handle], loc="upper left")

    plt.tight_layout()
    plt.savefig(output_folder / f"{error_col}.png")
    plt.show()